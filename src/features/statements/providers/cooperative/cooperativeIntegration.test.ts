...
const samplePath = process.env.COOP_STATEMENT_FIXTURE;

describe.skipIf(!samplePath)("Co-operative Bank local PDF integration", () => {
  it("detects, parses, and reconciles the authorized local sample", async () => {
    const loadingTask = getDocument({ url: pathToFileURL(samplePath!).href });
    ...
    expect(parser.getProvider()?.id).toBe("cooperative");
